...
condition of the Floquet-Lindblad stroboscopic map.
$\Gamma = p^{n-1}$ is \emph{proved} from partition function channel
counting: each of the $n$ gate channels admits $p^{n-1}$ states, giving
total gain $\Gamma = p^{n-1}$. Self-consistency
...
\paragraph{Floquet derivation of correction coefficients.}
Each correction is derived from the recursion's coupled Diophantine
structure, not selected from a scan.

\textbf{Proton ($\lambda^2$):} The correction arises from cross-solution
self-energy. The three Diophantine solutions couple through their
cyclotomic channels $\Phithree(p_i)$. The $(6,2)$ auxiliary mode
couples resonantly to the primary $(3,5)$ because
$X_1/X_3 = 60/12 = p_1 = 5$ (integer divisibility). The self-energy
correction is the channel-count ratio:
\[
  \Delta M_p = -\frac{\Phithree(p_3)}{\Phithree(p_1)}\,\lambda^2
  = -\frac{7}{31}\,\lambda^2
\]
The $(4,3)$ mode is off-resonance ($X_1/X_2 = 5/2$, non-integer) and
contributes at higher order. Over the common denominator $7{,}149{,}840
= 465 \times (p^3{-}1)^2$, the correction is exactly
$-\Phithree(2) \times np = -105$ in the numerator, closing the residual
to $0.033\ppb$.

\textbf{Alpha ($\lambda^3$):} Cross-sector $\lambda^1$ and $\lambda^2$
corrections to $\alpha$ vanish identically by deep saturation
($f''(x_s) < 10^{-42}$). The first non-vanishing self-coupling is at
$\lambda^3$ (cubic gate order $n = 3$), with coefficient equal to the
sum of gate and unity coupling channels:
$(n/p + 1) = (n{+}p)/p = 8/5$.

\textbf{Neutron ($\lambda^2$):} The isospin fine-structure correction is
$-2/(np^2)\,\lambda^2$, where $2$ is the isospin degeneracy factor and
$np^2$ is the standard RASP second-order vertex normalization.

\paragraph{Exhaustive scan verification.}
The Floquet-derived coefficients are independently confirmed by
exhaustive scan over all RASP-basis terms with $\{2,3,5,31\}$-smooth
denominators (\texttt{cuft-lambda-perturbation-derivation.py}). At each
$\lambda$-order, the derived coefficient is the unique minimum-complexity
RASP-atomic candidate: proton $-\Phithree(2)/\Phithree(5)$ (1~of~6
candidates, only RASP-atomic); alpha $-(n{+}p)/p$ (only RASP-atomic
$\lambda^3$ correction; competitor $-19/12$ has alien prime~19); neutron
$-2/(np^2)$ (simplest compound term; zero RASP-atomic candidates exist
at this order). The scan confirms the derivation---no competing
coefficient survives at any $\lambda$ order.
...
counting that produces $\Gamma = p^{n-1}$.
...
\begin{align*}
  ...
  \text{Crossing number:}    &\quad c(T(3,5)) = \min(n(p-1),p(n-1)) = 10 \\
...
\paragraph{Structural coherence conformance.}
The structural form of the recursion---emergence forced by nonterminality,
self-selected parameters via gain-coherence, hierarchical mass formula with
modular scale separation, and $\{2,3,5,31\}$ denominator closure as
information compression---conforms fully to the Structural Coherence Anchor
Specification (SC-AS v1.0, Carroll 2026~\cite{carroll2026}), an independently
developed formal specification of universal structural coherence conditions.
All 11 SC-AS primitives, all 10 axioms (from null exclusion through
observation), and all 9 admissibility rules find direct instantiation in the
RASP framework without modification. In particular, the gain-coherence
condition (Step~1) instantiates SC-AS Admissibility Rule~9 (nonterminality
under an admissible driver)---the same forcing mechanism that drives the
entire SC-AS axiom sequence. This conformance suggests the recursion's
effectiveness reflects structural coherence constraints that any admissible
physical system must satisfy.
...
Coherence Research, Missouri 501(c)(3) (2026).
DOI: 10.5281/zenodo.18039635 (SC-CORE), 10.5281/zenodo.18043770 (SC-AXIOM).
\url{https://github.com/Coherence-Research/structural-coherence}.
CC BY-ND 4.0.
